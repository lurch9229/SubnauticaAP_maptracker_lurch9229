...
function hasSeaglide()
  seaglide=Tracker:ProviderCountForCode("seaglide")*1
  return seaglide
end

function hasVehicles()
  vehiclebay=Tracker:ProviderCountForCode("vehiclebay")*1
  return vehiclebay
end

function hasSeamoth()
  seamoth=Tracker:ProviderCountForCode("seamoth")*1
  return seamoth
end

function hasPrawn()
  prawn=Tracker:ProviderCountForCode("prawn")*1
  return prawn
end

function hasCyclops()
  cyclops=Tracker:ProviderCountForCode("cyclops")*1
  return cyclops
end

function hasPool()
  pool=Tracker:ProviderCountForCode("pool")*1
  return pool
end

function hasModify()
  modify=Tracker:ProviderCountForCode("modify")*1
  return modify
end
...
function canget400()
  if (has ("$useSeaglide") and has ("rebreather"))
  or (has ("hightank") and has ("fins") and has ("rebreather") and has ("$canModify"))
...
function canget500()
  ...
  or (has ("$useSeamoth") and has ("rebreather") and has ("useSMD1") and has ("$"))
  or has ("$usePrawn")
  or has ("$useCyclops")
  then
    return 1
...
function canget750()
  ...
  or (has ("$useCylops") and has ("$useCD1") and has ("$canget500"))
  then
    return 1
  else
    return 0
...
function useSeamoth()
  if (hasSeaglide(3) and has ("$useVehicles"))
  then
    return 1
  else
    return 0
...
function usePrawn()
  if (hasPrawn(4) and has ("$useVehicles"))
  then
    return 1
  else
    return 0
...
function useCyclops()
  if (hasCyclops(9) and has ("$useVehicles"))
  then
    return 1
  else
    return 0
...
function canUpgrade()
  if (hasPool(2) and has ("console") and has ("builder"))
  then
    return 1
  else
    return 0
...
function canModify()
  if (hasModify(3) and has ("builder"))
  then
    return 1
  else
    return 0
...
function useSMD2()
  if (has ("$canModifyUpgrade") and has ("smd1") and has ("smd2") and has ("$magneACCESS"))
  then
    return 1
  else
    return 0
